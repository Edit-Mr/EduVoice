...
app.post("/login", requireAuth, async (req, res) => {
  ...
  if (!email || !password) {
    ...
    return Render(res, "login", req, 400, {
      email,
      message: "未輸入帳號或密碼",
    });
  }
  try {
    ...
    if (user) {
      ...
      console.log("準備發 JWT");
      const cookieInJWT = jwt.sign(
        { email, name, schoolId, school },
        JWT_SECRET,
        { expiresIn: "1d" }
      );
      try {
      ...
      return Render(res, "login", req, 401, {
        email,
        message: "帳號或密碼錯誤",
      });
    }
    ...
    return Render(res, "login", req, 500, {
      email,
...
app.post("/register", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (createdFlag) {
      ...
      setCookie(res, "userInfo", cookieInJWT);
      return Render(res, "signupResult", req, 200, {
        result: "註冊成功",
...
app.get("/i/:ruleId", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // console.log("ruleStatus", ruleStatus);
    return Render(res, "issue", req, 200, {
      ruleDetail,
      ruleStatus,
      totalinfo,
      ruleTags,
